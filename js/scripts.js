...
function initKrobs() {
    ...
    $("<span class='scale-callback transition2'></span>")
        .duplicate(6)
        .appendTo(".folio-overlay ");
    $(".box").hover(function() {
        var a = $(this);
...
$('.click-1').click(function() {
    $('.services-holder li  i , .img-click , .contact-info li i  , .policy-box  a  , .policy-box  i , .page-title h2 span , .error_message , .twitts li a , .num , .resume-head h4 , #testimonials-slider a , .twitter-holder h4 a , .subscribe-holder h4 , #contact-form input[type=text]:focus, #contact-form textarea:focus, input[type=email]:focus , #contact-social #contact-social  ul li a:hover  ul  li  a:hover , .side-site div i  ,  #success_page p , .post-meta li h6, .post-meta li a , .search-submit , .share-options ul li a:hover').css({
        'color': '#008000',
        'border-bottom': '4px',
    });
});
$('#1').click(function() {
    $('.button , nav  a span , .arrow-nav a , .fab-button , .fixed__link-a , .home-separator span , .about-button , .custom-skillbar , .services-holder li:hover , .serv-overlay , #options li.actcat, #options li:hover , .folio-overlay span, .popup-modal-dismiss  , #contact-form input[type="submit"] , #jprePercentage , .scroll-btn:after , .color-separator , .twitter-holder .customNavigation a , .testimonials-holder .customNavigation a , .clients-holder, .subscriptionForm input#submitButton , .contact-tooltip  , .scroll-nav a  , .about-image .customNavigation a ,.box-details h3:after , .webkit-scrollbar-thumb , .social-list li a:before , #project-page-button a , .tagcloud a , .pagination a , .media-holder .customNavigation a').css({
        'background': '#008000',
    });
});
